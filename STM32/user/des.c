...
void DoKey(uint8_t *s, uint8_t * C, uint8_t * D)
{
    uint8_t i, j;
    uint8_t  t[8];
...
void DoLeft(uint8_t * s, int16_t n)
{
    uint8_t  L, t, i;
...
void DoRight(uint8_t * s, int16_t n)
{
    uint8_t   i, l, l1;
    for (i = 0; i < n; ++i)
    {
...
void SetKey(int16_t encrypt, uint8_t * C, uint8_t * D, int16_t n, uint8_t * K)
{
    uint8_t i;
...
void E_expand(uint8_t * s, uint8_t * r)
{
    r[0] = ((s[3] & 0x01) << 7) | ((s[0] & 0xf8) >> 1) | ((s[0] & 0x18) >> 3);
    r[1] = ((s[0] & 0x07) << 5) | ((s[0] & 0x01) << 3) | ((s[1] & 0x80) >> 3) |	((s[1] & 0xe0) >> 5);
    r[2] = ((s[1] & 0x18) << 3) | ((s[1] & 0x1f) << 1) | ((s[2] & 0x80) >> 7);
    r[3] = ((s[1] & 0x01) << 7) | ((s[2] & 0xf8) >> 1) | ((s[2] & 0x18) >> 3);
    r[4] = ((s[2] & 0x07) << 5) | ((s[2] & 0x01) << 3) | ((s[3] & 0x80) >> 3) |	((s[3] & 0xe0) >> 5);
    r[5] = ((s[3] & 0x18) << 3) | ((s[3] & 0x1f) << 1) | ((s[0] & 0x80) >> 7);
}

void P_chang(uint8_t * s)
{
    uint8_t i;
    uint8_t  t[4];
    t[0] = ((s[1] & 0x01) << 7) |
...
uint8_t Find_S(uint8_t * s, int16_t ns)
{
    uint8_t col, num, index = 0;
    if (ns == 1 || ns == 5)
    {
...
/*************************************************
  Function:       // DesEncrypt(int16_t encrypt, uint8_t* datat, uint8_t* keyt)
...
*************************************************/
void des(unsigned char *datat, unsigned char *keyt, unsigned char encrypt)
{
    uint8_t  i, j;

    uint8_t  en_loop[16] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
    uint8_t  de_loop[16] = {1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1, 1};
    uint8_t  C0[4], D0[4], K[6];
    uint8_t  * L, * R,c;
    uint8_t  s1[9], s2[9],t[8],tx1[4],t2[6];
